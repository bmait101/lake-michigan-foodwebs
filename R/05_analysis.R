...
plot(IsotopesList$`yellow perch_2016_ne_4`)
...
df_mod <- df %>%
  ...
  separate(species, into = c("species","year","lake_region", "season"), sep = "_") %>%
  # group_by(species) %>%
  group_by(species,year, lake_region, season) %>%
  summarise(mass_g = mean(mass_g, na.rm = TRUE), .groups = 'drop') %>%
  # left_join(TP_data, by=c("species")) %>%
  left_join(TP_data, by=c("species","year", "lake_region", "season")) %>%
  mutate(
...
df_mod |> 
  ...
  geom_point(aes(color = year, shape = lake_region), size = 3, alpha = 0.5) +
...
df_mod %>%
  ...
  add_predicted_draws(models_sub[[best_model_num]], re_formula = NULL) %>%  
  ggplot(aes(x = Alpha_mode)) +
  stat_lineribbon(aes(y = .prediction), fill = "grey", .width = c(.95, .8, .5),alpha = 0.5) +
  # geom_point(
  #   data = df_mod, 
