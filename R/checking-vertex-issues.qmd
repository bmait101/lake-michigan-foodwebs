...
a_parabola <- function(x) (-3.94 * x^2) + (4.06 * x) + 2.75
p4 <- ggplot() +
  geom_vline(xintercept = 0, linewidth = 0.5, color = "grey50") +
  geom_hline(yintercept = 0, linewidth = 0.5, color = "grey50") +
...
# library(nlme)
# lme1 <- lme(fixed = TP_mode ~ poly(Alpha_mode, 2, raw = FALSE), 
#             random = ~ Alpha_mode | lake_region,
#             data = m_dat)
# lme2 <- lme(fixed = TP_mode ~ poly(Alpha_mode, 2, raw = TRUE), 
#             random = ~ Alpha_mode | lake_region,
#             data = m_dat)
# get_vert(fixef(lme1)[2], fixef(lme1)[3])
# get_vert(fixef(lme2)[2], fixef(lme2)[3])
# 
# with(m_dat, plot(Alpha_mode, TP_mode, pch = 16))
# pred_data = data.frame(Alpha_mode = seq(0, 1, .05))
# pred_data$TP_mode = predict(lme2, newdata = pred_data, level = 0)
# with(pred_data, lines(Alpha_mode, TP_mode))
# abline(v=get_vert(fixef(lme2)[2], fixef(lme2)[3]), lty=2)
# 
# a_parabola <- function(x) (fixef(lme2)[3] * x^2) + (fixef(lme2)[2] * x) + fixef(lme2)[1]
# ggplot() +
#   geom_vline(xintercept = 0, linewidth = 0.5, color = "grey50") +
#   geom_hline(yintercept = 0, linewidth = 0.5, color = "grey50") +
#   geom_function(fun = a_parabola, linewidth = 1) +
#   geom_vline(xintercept = get_vert(fixef(lme2)[2], fixef(lme2)[3]), linetype = "dashed") + 
#   geom_point(data = m_dat, aes(x = Alpha_mode, y = TP_mode))
...
# now with raw polys
mod_tmp1 <- brm(
  bf(TP_mode ~ poly(Alpha_mode, 2, raw=TRUE) + (1 | lake_region) + (1 | season)),
  m_dat, 
  control = list(adapt_delta=adapt_d, max_treedepth = tree_depth),
  seed=12345, chains=n_chains, iter=2000, thin=n_thin, cores=n_cores)

# now without poly random effects
mod_tmp2 <- brm(
  bf(TP_mode ~ poly(Alpha_mode,2) + (Alpha_mode|lake_region) + (Alpha_mode|season)),
  m_dat,
  control = list(adapt_delta=adapt_d, max_treedepth = tree_depth),
  seed=12345, chains=n_chains, iter=2000, thin=n_thin, cores=n_cores)

# now without poly random effects but raw
mod_tmp4 <- brm(
...
loo(m, mod_tmp, mod_tmp1, mod_tmp2, mod_tmp4)
get_vert2(m)
get_vert2(mod_tmp)
get_vert(4.24, -3.43)  #mod_tmp1
get_vert2(mod_tmp2)
get_vert(4.49, -3.6)  #mod_tmp4
...
pred <- mod_tmp4 |> add_epred_draws(newdata = newdata, re_formula = NULL)
p5 <- pred |> 
  ggplot(aes(x = Alpha_mode, y = .epred)) +
  geom_vline(xintercept = 0, linewidth = 0.5, color = "grey50") +
  geom_hline(yintercept = 0, linewidth = 0.5, color = "grey50") +
