...
data <- bind_rows(
  ...
  mutate(common_name = case_when(
    common_name %in% c("quagga", "zebra") ~ "dreissenids", 
    common_name == "deepwater sculpin" ~ "deep water sculpin", 
    common_name == "gizzed shad" ~ "gizzard shad", 
    common_name == "nine-spine stickleback" ~ "9 spine stickleback", 
    common_name == "shorthead river redhorse" ~ "shorthead redhorse", 
    TRUE ~ common_name
  ))
